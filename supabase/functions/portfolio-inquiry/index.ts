...
const allowedOrigins = (Deno.env.get('PORTFOLIO_ORIGINS') ?? '')
  .split(',')
...
function validate(input: unknown) {
  ...
  if (payload.phone && (payload.phone.length < 7 || payload.phone.length > 32)) {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(payload.desiredDate) || Number.isNaN(date.valueOf())) {
    errors.desiredDate = 'Choose a date.';
  }
  if (payload.message.length < 10 || payload.message.length > 3000) {
    errors.message = 'Enter a message between 10 and 3,000 characters.';
  }
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(payload.workspaceSlug)) {
    return { ok: false as const, errors: { message: 'This contact form is not configured.' } };
  }
...
Deno.serve(async (request: Request) => {
  ...
  if (Number(request.headers.get('content-length') ?? 0) > 24_000) {
    return respond(origin, { ok: false }, 413);
  }
  ...
  if (workspaceError || !workspace || workspace.status === 'closed') {
    return respond(origin, { ok: false }, 503);
  }
