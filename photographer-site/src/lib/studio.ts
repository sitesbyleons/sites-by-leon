...
export function createStudioClient(getToken: () => Promise<string | null>): SupabaseClient | null {
  const url = import.meta.env.PUBLIC_SUPABASE_URL;
  const key = import.meta.env.PUBLIC_SUPABASE_PUBLISHABLE_KEY;
  if (!url || !key) return null;
  return createClient(url, key, { accessToken: getToken, auth: { persistSession: false } });
}
...
export async function loadStudioSession(input: {
  ...
  const client = createStudioClient(input.getToken);
