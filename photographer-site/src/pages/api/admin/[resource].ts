...
async function nextSort(client: SupabaseClient, table: string, workspaceId: string, extra?: { column: string; value: string }) {
...
async function removeFiles(client: SupabaseClient, paths: Array<string | null | undefined>) {
  const unique = [...new Set(paths.filter((path): path is string => Boolean(path)))];
  if (unique.length) await client.storage.from('portfolio-images').remove(unique);
}
...
const route: APIRoute = async ({ request, locals, params, url }) => {
  ...
  if (!client) return Response.json({ message: 'Studio database is not configured.' }, { status: 503 });

  const workspaceResult = await client.from('client_workspaces').select('id').eq('slug', import.meta.env.SITE_WORKSPACE_SLUG ?? 'northline').maybeSingle<{ id: string }>();
  if (workspaceResult.error || !workspaceResult.data) return Response.json({ message: 'You do not have access to this studio.' }, { status: 403 });
  ...
  if (method === 'DELETE') {
    ...
    await removeFiles(client, paths.map((path) => path ? managedPath(workspaceId, path) : null));
  ...
  if (oldPath && oldPath !== newPath) await removeFiles(client, [managedPath(workspaceId, oldPath)]);
  return Response.json({ ok: true });
};
