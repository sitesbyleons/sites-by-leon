  ...
  return {
    ...
    async createRateLimitedInquiry(input: RateLimitedInquiryInput): Promise<DataResult<Record<string, unknown>[]>> {
      try {
        ...
        const text = `with allowance as (insert into ${quote('inquiry_rate_limits')} (${quote('workspace_id')}, ${quote('ip_hash')}, ${quote('window_started_at')}, ${quote('request_count')}, ${quote('updated_at')}) values ($1::uuid, $2, now(), 1, now()) on conflict (${quote('workspace_id')}, ${quote('ip_hash')}) do update set ${quote('request_count')} = case when ${quote('inquiry_rate_limits')}.${quote('window_started_at')} <= now() - interval '10 minutes' then 1 else ${quote('inquiry_rate_limits')}.${quote('request_count')} + 1 end, ${quote('window_started_at')} = case when ${quote('inquiry_rate_limits')}.${quote('window_started_at')} <= now() - interval '10 minutes' then now() else ${quote('inquiry_rate_limits')}.${quote('window_started_at')} end, ${quote('updated_at')} = now() where ${quote('inquiry_rate_limits')}.${quote('window_started_at')} <= now() - interval '10 minutes' or ${quote('inquiry_rate_limits')}.${quote('request_count')} < 5 returning ${quote('workspace_id')}), created as (insert into ${quote('studio_inquiries')} (${quote('workspace_id')}, ${quote('ip_hash')}, ${quote('name')}, ${quote('email')}, ${quote('phone')}, ${quote('desired_date')}, ${quote('message')}) select $1::uuid, $2, $3, $4, $5, $6, $7 from allowance returning *) select * from created`;
        return { data: await executeQuery(text, values), error: null };
      } catch (error) {
