  ...
  return {
    ...
    async createRateLimitedInquiry(input: RateLimitedInquiryInput): Promise<DataResult<Record<string, unknown>[]>> {
      try {
        ...
        const text = `with lock as (select pg_advisory_xact_lock(hashtextextended($1::text || ':' || $2, 0))), allowance as (select count(*) as recent_count from ${quote('studio_inquiries')} cross join lock where ${quote('workspace_id')} = $1 and ${quote('ip_hash')} = $2 and ${quote('created_at')} >= now() - interval '10 minutes' having count(*) < 5), created as (insert into ${quote('studio_inquiries')} (${quote('workspace_id')}, ${quote('ip_hash')}, ${quote('name')}, ${quote('email')}, ${quote('phone')}, ${quote('desired_date')}, ${quote('message')}) select $1, $2, $3, $4, $5, $6, $7 from allowance returning *) select * from created`;
        return { data: await executeQuery(text, values), error: null };
      } catch (error) {
